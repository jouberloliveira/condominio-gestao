...
public class MoradorController {
    ...
    @Autowired private MoradorService service;
    @Autowired private UnidadeService unidadeService;

    private void addEnums(Model m) {
        m.addAttribute("tipos", TipoMorador.values());
        m.addAttribute("simNaoList", SimNao.values());
    }

    @GetMapping
    public String list(Model m) {
        m.addAttribute("moradores", service.findAll());
        m.addAttribute("currentPage", "moradores");
        m.addAttribute("title", "Moradores");
        return "moradores/list";
    }

    @GetMapping("/new")
    public String newForm(Model m) {
        m.addAttribute("morador", new Morador());
        m.addAttribute("unidades", unidadeService.findAll());
        addEnums(m);
        m.addAttribute("currentPage", "moradores");
        m.addAttribute("title", "Novo Morador");
        return "moradores/form";
    }

    @GetMapping("/{id}/edit")
    public String editForm(@PathVariable Long id, Model m) {
        m.addAttribute("morador", service.findById(id));
        m.addAttribute("unidades", unidadeService.findAll());
        addEnums(m);
        m.addAttribute("currentPage", "moradores");
        m.addAttribute("title", "Editar Morador");
        return "moradores/form";
    }

    @PostMapping
    public String save(@Valid @ModelAttribute Morador mo, BindingResult r,
        ...
        try {
            service.save(mo);
            ra.addFlashAttribute("success", "Morador salvo com sucesso!");
    ...
    public String delete(@PathVariable Long id, RedirectAttributes ra) {
        try {
            ...
            ra.addFlashAttribute("success", "Morador excluído!");
        } catch (Exception e) {
            ra.addFlashAttribute("error", "Erro ao excluir morador");
        }
        return "redirect:/moradores";
    }
}
